...
// Master admin dashboard, ported into the client app. "View as" swaps the
// session for a read-only impersonation token and boots into the doctor's own
...
export default function AdminDashboard() {
  ...
  const viewAs = async (dentistId) => {
    setViewingId(dentistId);
    try {
      const { data } = await api.post(`/admin/impersonate/${dentistId}`);
      // Stash the admin session so "Exit view" returns here.
      const adminToken = localStorage.getItem("token");
      const adminUser = localStorage.getItem("user");
      if (adminToken) localStorage.setItem("adminToken", adminToken);
      if (adminUser) localStorage.setItem("adminUser", adminUser);
      localStorage.setItem("token", data.token);
      localStorage.setItem("viewAs", data.dentist?.clinicName || data.dentist?.name || "clinic");
      localStorage.removeItem("user"); // force a fresh /auth/me as the doctor
  ...
  const saveBilling = async (dentistId, startMonth, monthlyFee) => {
    setBusy(true);
    try {
      await api.put(`/admin/dentists/${dentistId}/billing`, { startMonth, monthlyFee });
      await load();
    } catch (err) {
  ...
  return (
    <div className="admin-ui">
      <header className="topbar">
        <div>
          <strong>MyDentalBooking</strong> <span className="muted">· Admin</span>
        </div>
        <div className="row">
      ...
      <main className="container">
        ...
        {summary && (
          <div className="cards">
            ...
            <Stat label="From PWA" value={`${summary.logins.pwa ?? 0} / ${summary.logins.success}`} />
        ...
        {tab === "logins" && (
          <>
            <div className="row between wrap" style={{ margin: "12px 0" }}>
              ...
              {events.length > 0 && (
                <button className="danger" onClick={clearAll} disabled={loading}>Clear all</button>
              )}
            </div>
                  ...
                  {loading ? (
                    ...
                    events.map((e) => (
                      <tr key={e._id}>
                        ...
                        <td>{e.identifier || "—"}</td>
                        <td>{e.role || "—"}</td>
                        <td>{e.success ? <span className="ok">Success</span> : <span className="bad" title={e.reason}>Failed</span>}</td>
                        <td>{e.pwa ? <span className="ok">PWA</span> : <span className="muted">Browser</span>}</td>
                        <td className="nowrap">{e.ip || "—"}</td>
                        <td title={e.userAgent}>{shortUA(e.userAgent)}</td>
                        <td>
                          <button className="link-danger" title="Delete this record" onClick={() => deleteEvent(e._id)}>✕</button>
                        </td>
                      </tr>
                    ))
                  )}
          ...
          <>
            <div className="row between wrap" style={{ margin: "12px 0" }}>
              <div className="row wrap">
                ...
                {[["all", "All"], ["installed", "On PWA"], ["not", "Not on PWA"]].map(([v, label]) => (
                  <button key={v} className={userPwa === v ? "chip on" : "chip"} onClick={() => setUserPwa(v)}>
                    {label}
                  </button>
                ))}
              </div>
        ...
        {tab === "enrollments" && (
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Clinic</th><th>Dentist</th><th>Contact</th><th>Status</th><th>Enrolled</th><th>Discovery ends</th><th></th></tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={7} className="muted">Loading…</td></tr>
                      ...
                      <tr key={c._id}>
                        ...
                        <td className="nowrap">
                          <button
                            ...
                            title="Open this doctor's own app (read-only)"
                          >
                ...
                <tbody>
                  {loading ? (
                    ...
                    invoices.map((iv) => {
                      ...
                      return (
                        <tr key={iv._id}>
                          <td>{iv.dentist?.clinicName || iv.dentist?.name || "—"}</td>
...
function BillingRow({ clinic, defaultFee, busy, onSave }) {
  ...
  const [fee, setFee] = useState(clinic.billing?.monthlyFee != null ? String(clinic.billing.monthlyFee) : "");
  const dirty =
  ...
  return (
    <tr>
      ...
      <td className="nowrap"><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></td>
      <td className="nowrap">
        <input
          type="number"
      ...
      <td className="nowrap">
        <button className="btn-sm" disabled={busy || !dirty} onClick={() => onSave(clinic._id, month, fee)}>Save</button>
      </td>
    </tr>
  );
}
